...
@Slf4j
class EmbeddedStandardJarScanner implements JarScanner {
	 ...
	 *
	 * @param scanType The type of JAR scan to perform. This is passed to the filter
	...
	public void scan(JarScanType scanType, ServletContext context, JarScannerCallback callback) {
		...
		if (isScanAllDirectories()) {
			try {
				URL url = context.getResource(WEB_INF_CLASSES_META_INF);
				if (url != null) {
					...
					URL webInfURL = context.getResource(WEB_INF_CLASSES);
					if (webInfURL != null) {
					...
					try {
						...
						if (f.isFile() && isScanAllFiles()) {
							...
							if (scanType == JarScanType.PLUGGABILITY) {
								...
								File metainf = new File(f.getAbsoluteFile() + File.separator + META_INF);
